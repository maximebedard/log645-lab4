...
void heat_dissipation_seq(int m, int n, float ***matrix, int *current, int np, float td, float h);
void heat_dissipation_par(int m, int n, float ***matrix, int *current, int np, float td, float h);

void matrix_init(int m, int n, float ***matrix);
void matrix_print(int m, int n, float ***matrix, int current);
...
int main(int argc, char const *argv[]) {
	...
	for (int k = 0; k < 2; k++) {
		matrix[k] = new float*[m];
		for (int i = 0; i < m; i++) {
			matrix[k][i] = new float[n];
		}
	}
	...
	double dt_seq = (end - start) / CLOCKS_PER_SEC;
	...
	double dt_par = (end - start) / CLOCKS_PER_SEC;
...
void heat_dissipation_seq(int m, int n, float ***matrix, int *current, int np, float td, float h) {
	...
	*current = 0;
	...
	for (k = 1; k < np; k++) {
		for (i = 1; i < m - 1; i++) {
			for (j = 1; j < n - 1; j++) {
				matrix[*current][i][j] = (1.0f - 4 * td / h*h) * matrix[1 - *current][i][j] +
...
void heat_dissipation_par(int m, int n, float ***matrix, int *current, int np, float td, float h) {
	...
	float* h_matrix = new float[m*n];//(float*)malloc(mem_size_A);
	float* j_matrix = new float[m*n];// (float*)malloc(mem_size_A);
	...
	*current = 0;
	...
	ret = clReleaseMemObject(e_matrix);
...
void matrix_zero(int m, int n, float ***matrix) {
	int i, j;
